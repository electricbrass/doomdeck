...
bool Backend::Initialize(const char* window_name, int width, int height,
    ...
    if (!available_renderers.empty()) {
        data->system_interface.LogMessage(
            Rml::Log::LT_INFO, Rml::CreateString("Available SDL renderers: %s",
                                                 available_renderers.c_str()));
    }

    if (renderer_name) {
        data->system_interface.LogMessage(
            Rml::Log::LT_INFO,
            Rml::CreateString("Using SDL renderer: %s", renderer_name));
    }
...
bool Backend::ProcessEvents(Rml::Context* context,
    ...
    auto GetDisplayScale = []() {
        return SDL_GetWindowDisplayScale(data->window);
    };
    ...
    while (has_event) {
        ...
        case event_key_down: {
            ...
            const Rml::Input::KeyIdentifier key =
                RmlSDL::ConvertKey(static_cast<int>(GetKey(ev)));
            ...
            if (key_down_callback &&
                !key_down_callback(context, key, key_modifier, native_dp_ratio,
                                   true)) {
                break;
            }
