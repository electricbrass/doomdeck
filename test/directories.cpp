...
TEST_CASE_METHOD(DirectoriesTestFixture, "Reject relative paths", "[directories]") {
    ...
    setenv(
        "XDG_DATA_DIRS", "relative:/absolute:/absolute/too:./relative:/absolute/again:relative/too",
        1
    );
    const auto dirs = dirs::Directories();
    REQUIRE(dirs.cache_home() == "/home/test/.cache");
    ...
    REQUIRE(dirs.state_home() == "/home/test/.local/state");
    REQUIRE_THAT(
        dirs.data_dirs(), RangeEquals(PathVec{"/absolute", "/absolute/too", "/absolute/again"})
    );
    REQUIRE_THAT(
        dirs.data_search_dirs(),
        RangeEquals(
            PathVec{"/home/test/.local/share", "/absolute", "/absolute/too", "/absolute/again"}
        )
    );
